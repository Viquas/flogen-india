...
- Sales CRM: `user_roles.role` now includes `'sales'` in addition to `'admin'`/`'client'`. Admins have implicit sales access. Sales routes live under `/sales` (route group `(sales)`), entry point is `/sales-login`. Use `requireSales()` from `lib/auth/require-sales.ts` in server actions.
- `call_logs` is append-only — never update or delete rows. The `trg_sync_project_sales_state` trigger maintains `projects.sales_status`, `sales_last_contact_at`, `sales_last_contact_by`, `sales_call_count`, and `sales_next_followup_at`. App code MUST NOT write to those `projects.sales_*` columns directly; always go through a `call_logs` insert.
- Sales conversion metrics are ground-truthed by `claims.paid_at IS NOT NULL`, NOT the salesperson-reported `'closed'` outcome. This prevents self-inflation and keeps sales/admin numbers aligned.
