...
async function executeDiscoverStage(run: BatchRun): Promise<void> {
  ...
  if (run.batch_id) {
    ...
    if (count && count > 0) {
      console.log(`[Autopilot] DISCOVER: batch ${run.batch_id} already has ${count} projects, skipping`)
  ...
  console.log(`[Autopilot] DISCOVER: found ${result.count} businesses, batch ${result.batchId}`)
}
...
async function executeEnqueueStage(run: BatchRun): Promise<void> {
  ...
  console.log(`[Autopilot] ENQUEUE: queued ${projectIds.length} projects for generation`)
...
async function executeGenerateStage(run: BatchRun): Promise<void> {
  ...
  while (true) {
    ...
    console.log(`[Autopilot] GENERATE: pending=${counts.pending}, processing=${counts.processing}, completed=${counts.completed}, failed=${counts.failed}`)

    // Check if all done
    if (counts.pending === 0 && counts.processing === 0) {
      console.log(`[Autopilot] GENERATE: all jobs finished`)
      break
    }
    ...
    if (stuckJobs.length > 0) {
      console.log(`[Autopilot] GENERATE: resetting ${stuckJobs.length} stuck jobs`)
      await supabase
        .from('queue_jobs')
    ...
    if (Date.now() - startTime > timeoutMs) {
      console.log(`[Autopilot] GENERATE: timeout reached, force-failing remaining jobs`)
...
async function executeFixStage(run: BatchRun): Promise<void> {
  ...
  if (!run.config.autoFixEnabled) {
    console.log(`[Autopilot] FIX: auto-fix disabled, skipping`)
  ...
  if (projectsToFix.length === 0) {
    console.log(`[Autopilot] FIX: no projects need fixing`)
    await advanceStage(run.id, 'scoring')
    return
  }

  console.log(`[Autopilot] FIX: fixing ${projectsToFix.length} projects (threshold: ${run.config.qualityThreshold})`)
  ...
  for (let i = 0; i < projectsToFix.length; i += CONCURRENCY) {
    ...
    console.log(`[Autopilot] FIX: progress ${i + batch.length}/${projectsToFix.length} (fixed: ${fixedCount})`)
  }

  console.log(`[Autopilot] FIX: completed. Fixed ${fixedCount}/${projectsToFix.length}`)
  await advanceStage(run.id, 'scoring')
}
...
async function executeScoringStage(run: BatchRun): Promise<void> {
  ...
  console.log(`[Autopilot] SCORING: complete. ${generated} generated, ${failed} failed, avg quality: ${avgQuality}`)
}
...
export async function runAutopilotPipeline(runId: string): Promise<void> {
  ...
  while (currentStage !== 'completed' && currentStage !== 'failed') {
    ...
    try {
      ...
      console.error(`[Autopilot] Stage ${currentStage} failed:`, message)
      await markFailed(runId, message)
      return
    }
  }

  console.log(`[Autopilot] Pipeline finished with stage: ${currentStage}`)
}
